...
**Regenerate demo keys after changing a definition** — `npm run keys:generate` in [demos/](demos/). `demos/src/keys.generated.d.ts` is generated from the definitions and is what makes `promptKeys`, `requirementKeys` and friends autocomplete and reject typos. It goes stale when a definition is added, removed, renamed, or re-keyed; `./test.sh` runs `keys:check` up front and refuses to proceed on a stale file. Generating needs `api/dist`. Note the dev stack deletes and rebuilds it on every change to `api/src` or `demos/src`, so run these when the stack is idle — mid-rebuild you get a confusing "could not resolve '@reqquest/api'" or a module-not-found instead.
