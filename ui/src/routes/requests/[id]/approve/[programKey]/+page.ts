...
export const load: PageLoad = async ({ params, depends }) => {
  ...
  const coalescedAppRequestData = coalesceAppRequestPrompts(appRequest, inlinePromptsWithData)
  depends('request:approve')
  if (!appRequest) throw error(404, 'App Request not found')
  return { appRequest: coalescedAppRequestData, programKey: params.programKey }
}
