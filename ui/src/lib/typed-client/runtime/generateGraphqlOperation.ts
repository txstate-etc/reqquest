...
export type Variables = Record<string, {
  value: any
    ...
    const fieldsSelection = fieldNames
      ...
      .map(f => {
        ...
        if (f.startsWith('on_')) {
          ...
          const typeMatch = /^on_(.+)/.exec(f)

          if (!typeMatch?.[1]) { throw new Error('match failed') }
...
export const generateGraphqlOperation = (
  ...
  return {
    ...
    variables: Object.keys(ctx.variables).reduce<Record<string, any>>(
      (r, v) => {
