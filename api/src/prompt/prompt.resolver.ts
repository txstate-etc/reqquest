...
export class RequirementPromptResolver {
  ...
  @FieldResolver(type => JsonData, { nullable: true, description: 'All the configuration data that could be relevant for this prompt. This includes its own config, and also the config data for any requirements and programs that are related to it.' })
  async configurationRelatedData (@Ctx() ctx: RQContext, @Root() requirementPrompt: RequirementPrompt) {
