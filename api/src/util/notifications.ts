...
export const applicationRescindNotifications: ApplicationRescindNotificationCB[] = [
  async (ctx, ar, application, reason) => {
    ...
    if (application.rescindedStatus === ApplicationRescindedStatus.RESCINDED) {
      await ctx.svc(MailService).sendmulti({ userIds: [ar.userInternalId], templateKey: 'application_rescinded', extra: { ...appConfig.emailConfig, programName: application.title, reason } })
    }
  },
  async (ctx, ar, application, reason) => {
    ...
    if (application.rescindedStatus === ApplicationRescindedStatus.RESTORED) {
      await ctx.svc(MailService).sendmulti({ userIds: [ar.userInternalId], templateKey: 'application_restored', extra: { ...appConfig.emailConfig, programName: application.title, reason } })
    }
  }
]
